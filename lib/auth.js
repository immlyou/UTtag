...
function error(res, message, status = 400, req) {
  cors(res, req);
  // In production, never leak raw 5xx messages to the client — Supabase error
  // strings can carry column names / SQL fragments. Log the real message
  // server-side and send a generic one to the client. 4xx is safe (validation).
  let clientMessage = message;
  if (status >= 500) {
    const method = req?.method || "?";
    const path = req?.url || req?.path || "?";
    console.error("[error]", status, method, path, "-", message);
    if (IS_PROD) clientMessage = "Internal server error";
  }
