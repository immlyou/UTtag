...
/**
 * Dual-mode authentication: accepts either an admin JWT or a tenant JWT.
 *
 * Returns one of:
 *   { kind: 'admin',  admin:  {id, username, role}, scopeClientId: null }
 *   { kind: 'tenant', user:   {id, email, role, client_id, ...}, scopeClientId: <uuid> }
 * or null if authentication failed (401/403 already written to res).
 */
async function dualAuth(req, res) {
  const auth = req.headers.authorization;
  if (!auth || !auth.startsWith("Bearer ")) {
    error(res, "Authorization required", 401, req);
    return null;
  ...
  if (payload.type === "tenant_user") {
    ...
    return { kind: "tenant", user: tenantUser, scopeClientId: user.client_id };
  }
  ...
  if (!admin) {
    error(res, "Admin not found", 403, req);
    return null;
  }

  return { kind: "admin", admin, scopeClientId: null };
}
